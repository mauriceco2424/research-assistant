...
- Modified principles: Template placeholders → concrete ResearchBase principles P1–P10
- Added sections:
  - Non-Functional Architecture & UX Constraints
  - Development Workflow & Spec Discipline
...
- Templates requiring updates:
  ...
  - ⚠ .specify/templates/commands/* (Directory not present; no action possible)
  - ⚠ README.md / runtime guidance (File not present; recommend adding later)
- Follow-up TODOs:
  - TODO(RUNTIME_GUIDANCE): Create a runtime development guidance doc that summarizes
    how features must demonstrate compliance with principles P1–P10.
-->

# ResearchBase Constitution
...
**Rationale**: ResearchBase is a desktop-first application. Users must be able
to trust that their libraries, notes, and derived artifacts remain local by
default, with network use being explicit, inspectable, and revocable.
...
**Rationale**: The LLM may "pull the strings" to orchestrate work, but users
retain control over what leaves their machine and which papers are acquired.
All network and acquisition behavior must be explainable and auditable.
...
**Rationale**: The master spec defines a dual-layer architecture so reports,
learning flows, and writing assistance can be regenerated from long-term
memory and user artifacts. This architecture must not be bypassed by ad hoc
state or opaque databases.
...
**Rationale**: Reports are a view over the AI-layer memory and paper library,
not a source of truth. Regenerability guarantees that the system stays
predictable and debuggable as the AI evolves.
...
**Rationale**: ResearchBase pushes complexity into AI orchestration and
structured reports. The UI must remain simple and predictable so researchers
can focus on thinking, not on managing screens and panels.
...
**Rationale**: The AI is allowed to orchestrate complex workflows but must not
perform hidden, irreversible changes. Users need a clear trail and the ability
to correct mistakes.
...
**Rationale**: ResearchBase is for serious research and writing. Misleading
citations or indistinguishable AI text undermines trust and violates academic
norms.
...
**Rationale**: The goal is to help the researcher actually understand and
retain material, not just generate text on command.
...
**Rationale**: The constitution, master spec, and feature specs must stay in
lockstep so that AI agents and humans are always working against the same
rules.
...
**Rationale**: Researchers may keep a Base alive for years. Extensibility must
not come at the cost of data safety or long-term usability.

## Non-Functional Architecture & UX Constraints

1. ResearchBase is a desktop-first application whose primary purpose is to
   help researchers organize, understand, and expand their literature, write
   and revise academic work, and learn material through interactive sessions,
   as described in `master_spec.md`.
2. The system MUST treat the chat interface as the primary control surface:
   any new capability that cannot be triggered and monitored via chat is
   considered non-compliant unless explicitly exempted under P9.
3. All complex visualizations (concept maps, networks, timelines, heatmaps)
   MUST be delivered as regenerable HTML reports rather than bespoke in-app
   UI components.
4. The Paper Acquisition Workflow described in the master spec MUST always
   follow this pattern:
   - AI proposes candidate papers with metadata and identifiers.
   - User selects which candidates to add and explicitly approves acquisition.
   - The app attempts DOI/URL-based metadata and open-access PDF retrieval.
   - Failures result in metadata-only entries marked as needing manual PDFs.
5. Voice input, if enabled, MUST route through the same orchestration and
   consent model as text chat (including approval for acquisitions and remote
   AI calls).

**Rationale**: This section ties the principles directly to the concrete
product shape in the master spec: desktop-first, chat-first, local-first,
with HTML reports as the primary surface for complexity.

## Development Workflow & Spec Discipline

1. Every non-trivial feature MUST have a feature specification based on
   `.specify/templates/spec-template.md`, including user stories, requirements,
   and success criteria that explicitly indicate compliance status for P1–P10.
2. Every implementation plan generated from `.specify/templates/plan-template.md`
   MUST include a "Constitution Check" that documents:
   - which principles are affected,
   - which are at risk of violation,
   - and any requested exemptions (mapped to P9 statuses).
3. Task lists generated from `.specify/templates/tasks-template.md` MUST
   include, where relevant, tasks to:
   - implement required logging and orchestration events (P6),
   - implement or verify local-first storage behavior (P1, P3),
   - implement regenerability and report re-generation (P4),
   - implement or verify chat-first interactions (P5),
   - implement acquisition approval flows (P2, non-functional constraints).
4. Pull requests that implement or change features MUST explicitly state:
   - which P1–P10 principles are impacted,
   - whether compliance is full, exempt, or blocked,
   - how the change aligns with `master_spec.md`.
5. Automated or semi-automated agents (such as SpecKit commands) MUST treat
   this constitution as the source of truth when generating plans, specs, and
   tasks and MUST not introduce patterns that contradict P1–P10.

**Rationale**: The constitution is only useful if it flows into day-to-day
work. This section binds the templates and review process to the principles.

## Governance

1. This constitution governs the ResearchBase project and supersedes any
   conflicting informal practices or historical patterns once ratified.
2. The canonical copy of the constitution is stored at
   `.specify/memory/constitution.md` in the repository and MUST be kept under
   version control.
3. Constitution versioning MUST follow semantic versioning:
   - MAJOR: Backward incompatible changes to governance or principle
     definitions (including removals).
   - MINOR: New principles or sections added, or materially expanded guidance.
   - PATCH: Clarifications, wording fixes, or non-semantic refinements.
4. Any proposal to change this constitution MUST include:
   - a description of the change,
   - the proposed version bump (MAJOR/MINOR/PATCH) with rationale,
   - an impact assessment on P1–P10 and on existing Bases.
5. All amendments MUST be reviewed and approved in the same repository that
   hosts the code, and the approval MUST be referenced in the commit message
   that updates this file.
6. Compliance reviews:
   - MUST occur at least once per major feature release or at each MAJOR/MINOR
     constitution bump, whichever comes first;
   - MUST audit at least: storage behavior (P1, P3), acquisition flows (P2),
     orchestration logging (P6), and report regenerability (P4).
7. If a feature cannot comply with a principle, this MUST be documented as an
   explicit exemption under P9 with a clear plan (or decision) for whether
   and when the exemption will be removed.

**Rationale**: Governance rules ensure that changes to ResearchBase remain
intentional, reviewable, and traceable, and that the constitution is a living
document tied to real decisions.
